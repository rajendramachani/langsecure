        ...
        for module_key, module_input in run_state.all_module_inputs.items():
            if module_key == stage:
                if module_key == "input":
                    ...
                    allow, denied_message = input_guardrails(  # noqa: F821
                        module_input["input"]
                    )
                    if allow is False:
...
class LangSecure(BaseModel):
    ...
    def __init__(self, **params):
        ...
        if self.policy_store is None:
            raise ValueError(
                "policy_store must be initialized to a local dir"
        ...
        if isinstance(self.policy_store, Path):
            ...
            raise ValueError(
                "policy_store with local directory is only supported."
                "Future versions will support remote store as well."
            )
